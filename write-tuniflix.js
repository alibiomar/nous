...
const content = `import { chromium, Browser, BrowserContext } from 'playwright';
...
export async function getSeries(slug: string): Promise<TuniflixSeason[]> {
  ...
  $('.se-c').each((_, seasonElement) => {
    const seasonName = $(seasonElement).find('.se-t').first().text().trim() || 'Season';
    const episodes: TuniflixEpisodeItem[] = [];

    $(seasonElement).find('li').each((__, episodeElement) => {
      const title = $(episodeElement).find('.epl-title').first().text().trim() || $(episodeElement).text().trim();
      const rawLink = $(episodeElement).find('a').first().attr('href');
      const link = normalizeToAbsoluteUrl(rawLink);
      episodes.push({ title, slug: extractSlugFromUrl(link), link });
    });

    seasons.push({ season: seasonName, episodes });
  });
...
fs.writeFileSync('e:/nous/lib/tuniflix.ts', content, 'utf-8');
console.log('Successfully wrote tuniflix.ts');
